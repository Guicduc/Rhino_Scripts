"""Split closed Rhino solids into print-bed-sized parts with XYZ peg/socket fittings.
...
Run with Rhino's Python script runner. The script:
  ...
  4. Adds fittings between neighboring parts on selected X/Y/Z split seams.
  5. Uses half-round fittings for X/Y seams so the flat side can sit near the
     print-bed side. Z seams use round vertical pegs/sockets.
...
DEFAULT_CONNECTION_AXES = "XYZ"
PRINT_BED_UP_AXIS = 2
OUTPUT_LAYER = "Split print parts with XYZ fittings"
LOG_FILE_NAME = "split_for_print_with_fittings_xyz_debug.log"
...
class _ProgressMeter(object):
    def __init__(self, label, total_steps):
        self.total_steps = max(int(total_steps), 1)
        self.current_step = 0
        self.enabled = False
        try:
            Rhino.RhinoApp.SetCommandPrompt(label)
            Rhino.UI.StatusBar.ShowProgressMeter(0, self.total_steps, label, True, True)
            self.enabled = True
        except Exception:
            self.enabled = False

    def step(self, label=None, amount=1):
        ...
        if self.enabled:
            try:
                Rhino.UI.StatusBar.UpdateProgressMeter(self.current_step, True)
            except Exception:
                pass
...
def _parse_connection_axes(text):
    if not text:
        raise ValueError("Expected at least one connection axis: X, Y, or Z")

    cleaned = text.upper().replace(",", "").replace(";", "").replace(" ", "")
    axes = set()
    for character in cleaned:
        if character not in "XYZ":
            raise ValueError("Unsupported connection axis '{0}'".format(character))
        axes.add("XYZ".index(character))
...
def _format_connection_axes(axes):
    return "".join(_axis_name(axis) for axis in range(3) if axis in axes)
...
def _add_fitting_geometry(
    ...
    _log(
        "Fitting setup: axes {0}, pin_radius {1}, pin_depth {2}, socket_radius {3}, socket_depth {4}, pins_per_seam {5}".format(
            _format_connection_axes(connection_axes),
    ...
    for axis in range(3):
        if axis not in connection_axes:
            _log("Skipping {0} fittings because the axis is disabled.".format(_axis_name(axis)))
            continue

        if counts[axis] < 2:
            _log("Skipping {0} fittings because the grid has only one cell on that axis.".format(_axis_name(axis)))
            continue
...
def main():
    ...
    connection_axes_text = rs.GetString(
        "Connection axes for fittings. Use X, Y, Z, XY, XZ, YZ, or XYZ",
        DEFAULT_CONNECTION_AXES,
    )
    if not connection_axes_text:
        _log("Run canceled while requesting connection axes.")
        return

    try:
        connection_axes = _parse_connection_axes(connection_axes_text)
    except Exception as error:
        rs.MessageBox("Could not read connection axes: {0}".format(error), 16, "Split for print")
        _log("Run stopped: invalid connection axes '{0}'.".format(connection_axes_text))
        return
    ...
    _log(
        "Run settings: bed {0}, clearance {1}, pin_radius {2}, pin_depth {3}, pins_per_seam {4}, connection_axes {5}, hide_originals {6}, doc_tolerance {7}".format(
    ...
    all_warnings = []
    all_oversized = []
    progress = _ProgressMeter("Preparing split for print", len(object_ids) * 5)

    try:
        for sequence, object_id in enumerate(object_ids, 1):
            progress.step("Reading object {0}".format(sequence))
            ...
            progress.step("Splitting {0}".format(label))
            ...
            progress.step("Creating {0} fittings on {1}".format(label, _format_connection_axes(connection_axes)))
            ...
            progress.step("Applying socket and peg booleans for {0}".format(label))
            ...
            progress.step("Adding output parts for {0}".format(label))
