...
sealed class NestingDialog : Dialog<bool>
{
    ...
    private PartInfo SelectedPart
    {
        get
        {
            return _partList.SelectedIndex >= 0 && _partList.SelectedIndex < _parts.Count ? _parts[_partList.SelectedIndex] : null;
        }
    }
...
internal static class NestingRhinoScript
{
    ...
    {
        get { return System.Environment.GetEnvironmentVariable("NESTING_DEBUG_LOG"); }
    }
    ...
    private static double Tolerance
    {
        ...
        {
            var doc = RhinoDoc.ActiveDoc;
            return doc != null ? doc.ModelAbsoluteTolerance : 0.01;
        }
    }
    ...
    private static List<PartInfo> BuildParts(List<Guid> ids, double spacing, RotationMode mode, bool allowMirroring, out Transform toWorld, out Transform fromWorld)
    {
        ...
        foreach (var id in ids)
        {
            ...
            foreach (var memberId in memberIds)
            {
                ...
                var curveObj = member != null ? member.Geometry as Curve : null;
                if (curveObj == null)
                    continue;
    ...
    public static List<OrientationInfo> BuildOrientations(List<Curve> curves2d, List<Curve> closedCurves2d, Point3d center, RotationMode mode, double spacing, bool allowMirroring)
    {
        ...
        foreach (var variant in OrientationVariants(mode, allowMirroring))
        {
            ...
            var planarBreps = Brep.CreatePlanarBreps(rotatedClosed, Tolerance);
            var materialBreps = planarBreps != null
                ? planarBreps.Where(b => b != null).Select(b => b.DuplicateBrep()).ToList()
                : new List<Brep>();
